...
//       if(mutation.addedNodes.length > 0){
//       console.log(mutation.addedNodes.length + " child node(s) have been removed" );
//       mutation.addedNodes.forEach(function (element) {
//           console.log("added id =" + element.id + "  node: " + element.cloneNode(false).outerHTML);
//           if(IsTRANSACTION_DETAILS_DIALOG(element)){
//             AddCalculators (element);
//           }
//       })
//       }
//       if(mutation.removedNodes.length > 0){
...
//           console.log("Removed id =" + element.id + " node: " + element.cloneNode(false).outerHTML);
//           IsTRANSACTION_DETAILS_DIALOG(element);
//       });
//       }
...
// function IsTRANSACTION_DETAILS_DIALOG(item){
//   if(item)
//   {
//     console.log(typeof item);
//     try{
...
//         if (TransactionDetailPage)
//         {
//             console.log("child Node Includes: " + TransactionDetailPage.cloneNode(false).outerHTML);
//             return true;
//         }
//     }catch(error){
//         console.error("An error occurred:", error.message);
...
// function AddCalculators(ModelNode){

//       let allAmountNodes = ModelNode.querySelectorAll('[sharedcomponentid="QAmountField"]');
//       //let allCalculatorButtonsMap = ConvertToMapByID(ModelNode.querySelectorAll(`[id^=${CalcButtonPrefix}]`));
      
//       for (let i = 0; i < allAmountNodes.length; i++) {
//         let currentInputDiv = allAmountNodes[i];
//         let currentButtonKey = CalcButtonPrefix + currentInputDiv.id;
//         //if (!allCalculatorButtonsMap.has(currentButtonKey)){
//           let inputBox = currentInputDiv.querySelector('input[type="text"]');

//           var focusEventHandler = createCustomOnFocusHandler(ModelNode);
//           inputBox.addEventListener('focus',focusEventHandler);
//           //let mybttn  = document.createElement("button");
//           //mybttn.id = currentButtonKey;
//           //let calcImage = document.createElement('img');
//           //calcImage.src = runtime.getURL('../images/calcIcon.jpg') ;
//           //calcImage.alt = 'Calc';
//           //mybttn.appendChild(calcImage);
...
// // function ConvertToMapByID(NodeList){
// //     let itemMap = new Map();
...
// function createCustomOnFocusHandler(currentModelParentNode){
//   return function(event) {
//     console.log('In Focus Event Object:', event);
//     console.log('For parent node:', currentModelParentNode);
//     let calculatorDiv  = document.createElement("input");
//     var LossFocusHandler = createCustomLossFocusHandler(event.target, currentModelParentNode);
//     calculatorDiv.addEventListener('blur',LossFocusHandler);
//     calculatorDiv.id = CalcDivName;
//     //;
//     calculatorDiv.type = 'text';
//     calculatorDiv.style.cssText = `width: 100px; height: 50px; position: absolute; background-color: red; z-index: ${ parseInt(window.getComputedStyle(currentModelParentNode).getPropertyValue('z-index'))+1}`;
//     event.target.parentNode.parentNode.insertAdjacentElement("afterend",calculatorDiv);
//     calculatorDiv.value = event.target.value;
//     requestAnimationFrame(function() {
//       // Now the element is likely rendered, so set focus
//       calculatorDiv.focus();
//     });
    
//     console.log('Calc Div Created:', calculatorDiv);
//   }
// }

// function createCustomLossFocusHandler(TargetInputBox,currentModelParentNode ){
//   return function(event) {
//     console.log('target inputbox', TargetInputBox);
//     console.log('Power Tool Calc Inputbox', event.target);
    
//     TargetInputBox.value = event.target.value;
    
   
//     event.target.parentNode.removeChild(event.target);
//      requestAnimationFrame(function() {

//       //remove event handler before setting focus then add it back
//       var focusEventHandler = createCustomOnFocusHandler(currentModelParentNode);
//       TargetInputBox.removeEventListener('focus',focusEventHandler)
//       TargetInputBox.focus();
//       TargetInputBox.addEventListener('focus',focusEventHandler);
